...
# Contains project(), run_named_scenario(), apply_passthrough_savings()
# Parameters are passed in; no utility-specific data is defined here.
# All parameters that currently reference globals (UTILITY_SAVINGS_SHARE,
# NWS_AVOIDANCE_RAMP_YEARS etc.) should continue to reference them as
# globals — do NOT change the parameter passing mechanism at this stage.
...
def run_named_scenario(utility_key, sdef):
    """
    Run shared savings under a named scenario's parameter overrides.

    Overrides Section 1 globals, calls project() (apply_freeze=False),
    restores globals, then applies two post-processing steps:
    ...
    """
    param_overrides = sdef['params']
    freeze_years    = param_overrides.get('RATE_FREEZE_YEARS', RATE_FREEZE_YEARS)

    saved = {}
    for pname, val in param_overrides.items():
        saved[pname] = globals()[pname]
        globals()[pname] = val

    p  = utility_data[utility_key]
    sh = project(p, 'shared', False)   # apply_freeze=False; freeze handled below
    ...
    if sdef['freeze'] and freeze_years > 0:
        ...
        catchup_end  = freeze_end + RATE_FREEZE_RECOVERY_YEARS
    ...
    cap_cost_start = (CAPACITY_COST_START[utility_key]
    ...
    pt_df = pd.DataFrame({'year': YEARS, 'pt_monthly_bill_reduction': pt_monthly})
    ...
    upy = sdef.get('upfront_passthrough_years', UPFRONT_PASSTHROUGH_YEARS)
    if upy > 0:
...
def apply_passthrough_savings(results, utility_key, utility_data):
    """
